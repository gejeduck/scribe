defmodule SocialScribeWeb.MeetingLive.CrmModalComponent do
  @moduledoc """
  Live component for updating CRM contacts with AI-suggested field values.

  Works with any CRM provider (HubSpot, Salesforce) via CrmApi.
  """
  ...
  def handle_event("contact_search", %{"value" => query}, socket) do
    ...
    if String.length(query) >= 2 do
      ...
      send(self(), {:crm_contact_search, query, socket.assigns.credential})
      {:noreply, socket}
    else
  ...
  def handle_event("select_contact", %{"id" => contact_id}, socket) do
    ...
    if contact do
      ...
      send(self(), {:generate_crm_suggestions, contact, socket.assigns.meeting, socket.assigns.credential})
      {:noreply, socket}
    else
  ...
  def handle_event("apply_updates", %{"apply" => selected, "values" => values}, socket) do
    updates =
      ...
      |> Enum.reduce(%{}, fn field, acc ->
        value = Map.get(values, field, "") |> String.trim()
        Map.put(acc, field, value)
      end)

    case CrmFieldValidator.validate(updates) do
      :ok ->
